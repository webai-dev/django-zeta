...
from .models import User, Group, UserRelation
...
class UserAdmin(VersionAdmin):
    ...
    @sensitive_post_parameters_m
    def user_change_password(self, request, myid, form_url=''):
        if not self.has_change_permission(request):
            raise PermissionDenied
        user = self.get_object(request, unquote(myid))
        if user is None:
            raise Http404(
                _('%(name)s object with primary key %(key)r does not exist.')
                % {'name': self.model._meta.verbose_name, 'key': escape(myid),}
            )
